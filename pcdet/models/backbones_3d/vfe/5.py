...
class VoxelFeature_TA(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = torch.cat([paca1, x], dim=2).permute(0,2,1).unsqueeze(-1).contiguous() #out1:[25336, 32, 22]>[25336, 22, 32,1]
        # out1 = paca1_feat.permute(0,2,1).unsqueeze(-1).contiguous() #out1:[25336, 32, 22]>[25336, 22, 32,1]
...
class PillarVFE_TA_paca(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        features *= mask
        features = self.VoxelFeature_TA(features)
