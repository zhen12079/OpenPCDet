...
def points_in_boxes3d(points, boxes3d):
    """
    Args:
        points: (num_points, 3 + C)
        boxes3d: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center, each box DO NOT overlaps
    ...
    point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(points[:, 0:3], boxes3d)
    points_scene = points[point_masks.sum(dim=0) == 0]
    # print("point_masks.shape", point_masks.shape)
    # print("points.shape", points1.shape)
    points_boxes = []
    for i in range(point_masks.shape[0]):
        points_boxes.append(points[point_masks[i,:] != 0])
        # print(point_masks[i,:])
        # print(points[point_masks[i,:] != 0].shape)

    # print("points_box.shape", points_box.shape)
    # quit()

    return points_scene.numpy() if is_numpy else points, points_boxes,
