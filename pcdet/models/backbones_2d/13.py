...
class MixSegNet(nn.Module):
    def __init__(self,
        ...
        self.mixsegnet = nn.Sequential(
            Rearrange('b c (h p1) (w p2 ex) -> b (p1 p2 c) (h w) ex', p1 = patch_size, p2 = patch_size, ex = 1),#[b,8192,324,1]
            nn.Conv2d((patch_size ** 2) * channels, dim, kernel_size=1),#[b,512,324,1]
            Permute([0,2,3,1]),
            *[nn.Sequential(
                PreNormResidual(dim, FeedForward_conv2d(num_patches, expansion_factor)),#[b,324,1,512]
                Permute([0,3,2,1]),
                PreNormResidual(324, FeedForward_conv2d(dim, expansion_factor)),#[b,512,1,324]
                Permute([0,3,2,1]),
